...
export function handleDocsMarkdown(req: IncomingMessage, res: ServerResponse): boolean {
  ...
  if (req.url?.startsWith('/assets/docs/') && req.url?.endsWith('.md')) {
    const urlWithoutQuery = req.url.split('?')[0];
    // 移除 /assets/docs/ 前缀和 .md 后缀
    const docPath = urlWithoutQuery.slice(13, -3); // 移除 '/assets/docs/' 和 '.md'
    
    // 对路径进行 URL 解码
    const decodedDocPath = decodeURIComponent(docPath);
    
    const mdPath = path.resolve(process.cwd(), 'assets/docs', decodedDocPath + '.md');
  ...
  // 处理 /assets/libraries/*.md
  if (req.url?.startsWith('/assets/libraries/') && req.url?.endsWith('.md')) {
    const urlWithoutQuery = req.url.split('?')[0];
    // 移除 /assets/libraries/ 前缀和 .md 后缀
    const libraryPath = urlWithoutQuery.slice(18, -3); // 移除 '/assets/libraries/' 和 '.md'
    
    // 对路径进行 URL 解码
    const decodedLibraryPath = decodeURIComponent(libraryPath);
    
    const mdPath = path.resolve(process.cwd(), 'assets/libraries', decodedLibraryPath + '.md');

    console.log('[虚拟HTML] 请求 library markdown:', req.url, '-> 路径:', mdPath, '存在:', fs.existsSync(mdPath));
    ...
    if (fs.existsSync(mdPath)) {
      try {
        ...
        res.statusCode = 200;
        res.end(content);
        console.log('[虚拟HTML] ✅ 返回 library markdown:', req.url);
        return true;
      } catch (err) {
        console.error('[虚拟HTML] ❌ 读取 library markdown 失败:', err);
      }
    } else {
